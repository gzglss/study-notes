...
apply_gradients、_do_use_weight_decay、_get_variable_name
MutilGPUAdamWeightDecayOptimizer：_prepare、_create_slots、_apply_dense、_resource_apply_dense、
_apply_sparse_shared、_apply_sparse、_resource_scatter_add、_resource_apply_sparse、_do_use_weight_decay
"""

# def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, current_task_name, use_distill=False):
...
#     # freeze other layers
#     if use_distill:
#         tvars = [tvar for tvar in tvars if tvar.name.startswith("student")]
#     else:
...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu=False, use_h=False):
    """Creates an optimizer training op."""
    global_step = tf.train.get_or_create_global_step()
