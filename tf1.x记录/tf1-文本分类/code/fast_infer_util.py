...
# build_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../build/build')

# build_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), './fastlib')
build_path = "/fds/utils/fastlib"

transformer_op_module = tf.load_op_library(
    os.path.join(build_path, 'libtf_fastertransformer.so'))


# transformer_op_module = tf.load_op_library( '/home/work/.cache/bazel/_bazel_work/108d1b864fa0035656052123fbe59709/execroot/tf_serving/bazel-out/k8-opt/bin/external/org_tensorflow/tensorflow/fastertransformer/bert_transformer_op.so' )
...
def fast_transformer_model_trans(input_tensor,
    ...
    for layer_idx in range(num_hidden_layers):
        with tf.variable_scope("layer_%d" % (layer_idx + int(offset) - 1)):
            ...
            tf.Print(layer_input, ['layer_input: ', layer_input])
            layer_output = transformer_op_module.bert_transformer(
                layer_input,
                layer_input,
                trainable_vars[0], trainable_vars[2], trainable_vars[4], trainable_vars[1], trainable_vars[3],
                trainable_vars[5],
                attention_mask,
                trainable_vars[6], trainable_vars[7], trainable_vars[8], trainable_vars[9], trainable_vars[10],
                trainable_vars[11],
                trainable_vars[12], trainable_vars[13], trainable_vars[14], trainable_vars[15],
                tf.tile([[1], [2], [3]], [1, fast_list_tensor[0]]), tf.tile([[1], [2], [3]], [1, fast_list_tensor[1]]),
                head_num=num_attention_heads, size_per_head=attention_head_size)
            # batch_size=pbatch_size, from_seq_len=pseq_length, to_seq_len=pseq_length, head_num=num_attention_heads, size_per_head=attention_head_size)
